...
export default {
  ...
  methods: {
    ...
    expressionChange() {
      ...
      if (ruleArr[3] === '0') {
        this.resultList = ['日必须介于1和31之间']
        this.isShow = true
        return
      } else if (ruleArr[4] === '0') {
        this.resultList = ['月必须介于1和31之间']
        this.isShow = true
        return
      } else if (ruleArr[3] !== '?' && ruleArr[5] !== '?') {
        this.resultList = ['不支持同时指定星期几和几月参数。']
        this.isShow = true
        return
      } else if (ruleArr[3] === '?' && ruleArr[5] === '?') {
        this.resultList = ['日和星期（周）中至少指定一个。']
        this.isShow = true
        return
      }
      // 用于记录进入循环的次数
        ...
        goMouth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
          ...
          goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
            ...
            if (this.dayRule == 'lastDay') {
              ...
              // if (Array.indexOf(this.dayRuleSup, thisWeek) < 0) { bug修改
