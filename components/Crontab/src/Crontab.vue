<template>
  <div>
    ...
    <div class="popup-main">
      ...
      <CrontabResult :ex="contabValueString" @getResultList="getResultList"></CrontabResult>
...
export default {
  ...
  name: 'vcrontab',
  props: ['expression', 'hideComponent', 'resultList'],
  methods: {
    shouldHide(key) {
    ...
    changeRadio(name, value) {
      ...
      if (arr.includes(name)) {
        if (value === '*') {
          ...
          insVlaue = 4
          this.$refs[refName].checkboxList = value.split(',').map(item => parseInt(item))
        }
      } else if (name == 'day') {
        if (value === '*') {
          insVlaue = 1
        ...
        if (value == '') {
          ...
          insVlaue = 3
          this.$refs[refName].cycle01 = value.split('-')[0]
          this.$refs[refName].cycle02 = value.split('-')[1]
        } else if (value.indexOf('/') > -1) {
          insVlaue = 4
          this.$refs[refName].average01 = value.split('/')[0]
          this.$refs[refName].average02 = value.split('/')[1]
        } else {
          this.$refs[refName].checkboxList = value.split(',').map(item => parseInt(item))
          insVlaue = 5
        }
    ...
    getResultList(val) {
      if (
        val[0] === '日必须介于1和31之间' ||
        val[0] === '月必须介于1和31之间' ||
        val[0] === '不支持同时指定星期几和几月参数。' ||
        val[0] === '日和星期（周）中至少指定一个。' ||
        val[0] === '没有达到条件的结果！'
      ) {
        this.resultList.value = []
      } else {
        this.resultList.value = val
      }
    }
  },
  computed: {
  ...
  watch: {
    contabValueString: {
      ...
      handler(newQuestion, oldQuestion) {
        this.$emit('getValue', newQuestion)
      }
    },
    expression: {
      handler(val) {
